...
// In dev (Vite), BASE_URL is '/', and we rely on the dev proxy for '/backend/api'.
// In production when hosted at '/capstone2/dist/', BASE_URL is '/capstone2/dist/' and API lives at '/capstone2/backend/api'.
export const API_BASE = (() => {
  const base = (import.meta && import.meta.env && import.meta.env.BASE_URL) || '/';
  // If Vite injected a project subpath (e.g., '/capstone2/dist/'), use it
  const fromBase = base.match(/^\/(.+?)\/dist\/$/);
  if (fromBase) {
    return `/${fromBase[1]}/backend/api`;
  }
  ...
  if (base === './' && typeof window !== 'undefined') {
    ...
    if (m && m[1]) {
      return `${m[1]}/backend/api`;
    }
  }
  // Fallback: root-level backend
  return '/backend/api';
})();
